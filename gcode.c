...
FLASHMEM __attribute__((weak)) bool cc_enable (gc_ccomp_t *comp_data, coord_data_t *position)
{
...
FLASHMEM bool gc_modal_state_restore (gc_modal_snapshot_t *snapshot)
{
    ...
    if((ok = !!snapshot && !ABORTED)) {
        ...
        if(actions->command.G7) // ?? add state validation++
            cc_enable(&gc_state.modal.cutter_comp, (coord_data_t *)gc_state.position);
#endif
...
// TODO: Recheck spindle running in CCS mode (is_rpm_pos_adjusted == On)?
    ...
    if(command_words.G7) {
        ...
        if(!cc_enable(&gc_block.modal.cutter_comp, (coord_data_t *)gc_state.position)) {
